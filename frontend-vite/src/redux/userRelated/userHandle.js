...
export const loginUser = (fields, role) => async (dispatch) => {
    ...
    try {
        const url = `${import.meta.env.VITE_BASE_URL}/${role}Login`;
        ...
        if (result.data && result.data.role) {
            dispatch(authSuccess({ user: result.data, role: result.data.role }));
        } else {
            dispatch(authFailed('Invalid response from server'));
        }
    } catch (error) {
